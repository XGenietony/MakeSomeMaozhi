...
class Assistant(object):
    ...
    def get_single_item_stock(self, sku_id, num, area):
        ...
        # url = 'https://api.m.jd.com/stocks'type=
        url = "https://api.m.jd.com/stocks?type=getstocks&skuIds=" + str(sku_id) +  "&area=" + str(area_id) + "&appid=item-v3&functionId=pc_stocks&_=" + str(int(time.time() * 1000))
        ...
        headers = {'User-Agent': self.user_agent, "Origin": self.origin, "Referer": self.referer, "cookie": self.cookie}

        # resp_text = ''

        # logger.error("payload finding", payload, headers, self.timeout)
    ...
    def get_multi_item_stock_new(self, sku_ids, area):
        ...
        url = "https://api.m.jd.com/stocks?type=getstocks&skuIds=" + str(self.sku_id) +  "&area=" + str(area_id) + "&appid=item-v3&functionId=pc_stocks&_=" + str(int(time.time() * 1000))
        ...
        try:
            resp_text = requests.get(url=url, params=payload, headers=headers, timeout=self.timeout)#.text
            data = resp_text.json()
            stock_info = data.get(str(self.sku_id))
            sku_state = stock_info.get('skuState')  # 商品是否上架
            stock_state = stock_info.get('StockState')  # 商品库存状态：33 -- 现货  0,34 -- 无货  36 -- 采购中  40 -- 可配货
            # stock = True
